...
void UUPSPatrolVisualizer::EnableInGameVisualization(bool bEnable)
{
	...
	if (GetWorld())
	{
		...
		{
			patrolPointIterator->GetBillboardComponent()->SetHiddenInGame(!bEnableVisualizationInGameMode);
		}
	}
}
		...
		if (pathData.FirstPoint.IsValid() && pathData.SecondPoint.IsValid())
		{
			if (needDrawPathPoints(pathData))
			{
				for (int32 pathPointIndex = 0; pathPointIndex < pathData.PathPoints.Num() - 1; pathPointIndex++)
				{
					...
					DrawRouteLine(startLine, endLine, pathData.bPathIsValid ? UUPSSettings::Get()->PathPointsColor : UUPSSettings::Get()->ErrorLineColor, pathData.bPathIsValid ? UUPSSettings::Get()->DefaultLinesThickness : UUPSSettings::Get()->ErrorLinesThickness);
				}
			}
		}
	}
...
{
	EnableInGameVisualization(false);
	bIsPIE = false;
